...
class State:
    ...
    def __eq__(self, other: 'State'):
        return self._pakkuman == other._pakkuman and \
               self._ghost == other._ghost and \
               self._ghost_dir == self._ghost_dir and \
               self._dots == other._dots

    def __hash__(self):
        return hash((self._pakkuman, self._ghost, self._ghost_dir, self._dots))
    ...
    @property
    def _lost(self):
        return self._pakkuman == self._ghost
    ...
    def _possible(self, loc):
        ...
        if not self.__oob(x - 1, y):
            possible.add(Pakkuman.Action.Up)
        if not self.__oob(x + 1, y):
            possible.add(Pakkuman.Action.Down)
        if not self.__oob(x, y - 1):
            possible.add(Pakkuman.Action.Left)
        if not self.__oob(x, y + 1):
            possible.add(Pakkuman.Action.Right)
        return possible

    @staticmethod
    def _do_action(loc, action):
        x, y = loc
        if action == Pakkuman.Action.Up:
            return x - 1, y
        if action == Pakkuman.Action.Down:
            return x + 1, y
        if action == Pakkuman.Action.Left:
            return x, y - 1
        return x, y + 1
    ...
    def _move(self, action):
        ...
        target_x, target_y = State._do_action(self._pakkuman, action)

        if self.__oob(target_x, target_y):
            pakkumann = self._pakkuman
        else:
            pakkumann = (target_x, target_y)

        dots = set(self._dots)
        if pakkumann in dots:
            dots.remove(pakkumann)
        ...
        def calc_dist(d):
            ...
            return math.hypot(p[0] - pakkumann[0], p[1] - pakkumann[1])
        ...
        if self._pakkuman == new_ghost and pakkumann == self._ghost:
            x1, y1 = self._pakkuman
            x2, y2 = self._ghost
            pakkumann = new_ghost = (x1 + x2) / 2, (y1 + y2) / 2
    ...
    def _get_actions(self):
        ...
        if self._lost or self._won:  # terminal
            return set()
        poss = set()
        all_poss = self._possible(self._pakkuman)
        for i in all_poss:
            poss.add(i)
            rev = 0b10 ^ i
            if rev in all_poss:
...
class GUI(tk.Tk):
    ...
    def __init__(self, grid, agent, init_state: State):
        ...
        self.__agent = agent
        self.__last_action = Pakkuman.Action.Right
    ...
    def play(self):
        ...
        while True:
            ...
            if isinstance(reward, bool):
                if reward:
                    print(f'Pakkuman won! Episode reward {total}')
                else:
                    print(f'Pakkuman lost! Episode reward {total}')
                return
            total += reward
            self.__update_score(total)
...
def simple_extractor(state, action):
    ...
    next_loc = state._do_action(state._pakkuman, action)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Gridworld GUI for Q-Learning',
    ...
    if args.agent == 'q':
        agent = agents.QLearningAgent(Pakkuman, args.discount, args.learning_rate, args.epsilon)
    else:
        extractor = globals()[args.feature + '_extractor'] # function quote
        agent = agents.ApproximateQAgent(
            Pakkuman, args.discount, args.learning_rate, args.epsilon, extractor=extractor)
